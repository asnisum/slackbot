# Slackbot — Slack 대화 분석기

Slack 스레드 URL을 붙여넣으면 대화를 자동 분석하여 **논의 내용, 결정 사항, 액션 아이템**을 추출해주는 Claude Code 플러그인입니다. 채널의 일별 대화 요약도 지원합니다.
...
- 하루치 채널 대화를 한눈에 파악하고 싶은 분

## 주요 기능

- **자동 스레드 분석**: Slack 스레드 URL만 붙여넣으면 전체 대화를 가져와 AI가 분석
- **스레드 업데이트**: 이미 분석한 스레드에 새 메시지가 추가되면 재분석 (이전 분석은 자동 백업)
- **채널 일별 요약**: 특정 채널의 특정 날짜 대화를 토픽별로 수집/분석
...
```
/slackbot:daily C0123456789 오늘
/slackbot:daily C0123456789 어제
/slackbot:daily C0123456789 2026-03-15
```

또는 "채널 오늘 대화 정리해줘", "daily summary" 등으로 자연어 트리거도 가능합니다.
...
├── README.md                  # 최신 요약 + 액션 아이템
├── README.2026-03-18.md       # 이전 분석 백업 (재분석 시)
...
**채널 일별 요약**:
```
~/.slackbot/{channel-id}/daily/{YYYY-MM-DD}/
├── README.md          # 일별 요약
├── conversation.md    # 원본 대화록
└── metadata.json      # 분석 메타데이터
```
