...
> **핵심 원칙**: 원본 JSON 먼저 저장 → 로컬 원본 기반 가공/분석
...
└── {CHANNEL_ID}/
    ...
    ├── daily/
    │   └── {YYYY-MM-DD}/
    │       ├── raw_messages.json   # ★ MCP 응답 원본 (최우선 저장)
    │       ├── conversation.md    # 포맷팅된 대화 (원본 기반 생성)
    │       ├── README.md          # 일별 요약 (원본 기반 생성)
    │       └── metadata.json      # thread_ids, thread_count 포함
    ├── {thread_ts}/               # 스레드 개별 저장
    │   ├── raw_replies.json       # ★ MCP 응답 원본 (최우선 저장)
    │   ├── README.md              # 스레드 요약
